// Encoder pins
#include <PinChangeInterrupt.h>  // For pin change interrupts
...
// Filter settings
const float ALPHA = 0.1;  // Smoothing factor (0.1-0.3 works well)
float smoothedRPMr = 0;    // Smoothed RPM value
float smoothedRPMl = 0;    // Smoothed RPM value
...
//global variables for storing pitch and roll
...
void setup() {
 ...
 // Timer for speed calculation
  uint8_t oldSREG = SREG;  // Save interrupt state
                    // Disable interrupts
 // Configure Timer0 (CTC mode, 1024 prescaler)
...
void CalculateReferenceRPM()
{
  //simple feedforward way to convert the pitch,roll into PWM for motors
  ...
  //these are Reference RPM values
...
void computePID() 
{
  ...
  //if the motor is not capable of achiving speeds the integral part keeps growing.this limits that behavior. 
